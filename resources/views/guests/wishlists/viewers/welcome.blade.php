...
<head>
    ...
    <title>{{ __('Join :wishlist on Snowbody Knows', ['wishlist' => $wishlist->name]) }}</title>
...
<body>
    ...
    <div class="mx-auto mt-12 px-4 max-w-sm">
        <h1 class="font-semibold text-center text-gray-900 text-xl"> {{ $wishlist->user->name }} sent you their wishlist! </h1>
        <p class="mt-4 text-center text-gray-600 text-lg"> Create an account or login to see their wishes, check items off, or even create a list of your own.</p>
    </div>
    <div class="mt-12 text-center w-full">
        <x-button-primary href="{{ route('register', ['wishlist' => $wishlist->invite_code]) }}">Create an account</x-button-primary>
        <x-button-secondary href="{{ route('login', ['wishlist' => $wishlist->invite_code]) }}">Login</x-button-secondary>
    </div>
</div>
<script>
